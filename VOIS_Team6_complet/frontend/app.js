// app.js (versiune curățată & corectată)
...
function initLogin() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (typeof data.aprobat !== "undefined") localStorage.setItem("aprobat", data.aprobat ? "true" : "false");
      if (String(localStorage.getItem("is_admin")) === "true") {
      location.href = "admin.html";
      } else if (String(localStorage.getItem("aprobat")) === true) {
      location.href = "employee.html";
      } else {
      errEl.textContent = "Contul tău nu a fost aprobat încă de un administrator.";
}
...
/* Users listing, role & approve helpers */
function roleLabel(u) {
  // backend uses is_admin and is_employee flags; adapt if you used different names
  if (u.is_admin) return "Admin";
  if (u.is_employee) return "Angajat";
  if (u.aprobat) return "Angajat";
  return "Neaprobat";
}
...
async function loadUsers() {
  ...
  allUsers.forEach((u) => {
    ...
    let actionsHtml = "";
    // approval button if not employee/approved yet
    if (!u.is_admin && !u.aprobat) {
      actionsHtml += `<button class="approveBtn" data-id="${u.id}">Aprobă cont</button>`;
    }
    if (u.aprobat && !u.is_admin) actionsHtml += `<button class="promoteBtn" data-id="${u.id}">Fă admin</button>`;
...
/* Endpoint detection helpers (fallbacks for different backends) */
...
async function detectEndpoints() {
      ...
      Object.keys(paths).forEach(p => {
        ...
        ops.forEach(m => {
          ...
          if (low.match(/\/users\/\{?id\}?$/) || low.match(/\/users\/\{?user_id\}?$/)) {
            ...
            const hasEmployee = JSON.stringify(schema).includes("is_employee") || JSON.stringify(schema).includes("approved");
            if (hasAdmin) {
  ...
  const candRole = [
    ...
    { m: "POST",  url: "/users/make-admin/{id}", type: "promote" },
    { m: "POST",  url: "/users/remove-admin/{id}", type: "demote" }
  ];
  const candApprove = [
    { m: "PATCH", url: "/users/{id}", type: "patch_flags" },
...
/* change role (promote/demote) - tries many fallbacks */
...
/* approve user - prefer direct endpoint /users/{id}/approve (PUT) then fallback to detection */
async function approveUser(userId) {
  // 1) direct (preferred)
  try {
    const rDirect = await fetch(`${API_BASE}/users/${userId}/approve`, {
      method: "PUT",
      headers: { ...authHeaders(), "Content-Type": "application/json" }
    });
    if (rDirect.ok) {
  ...
  // 2) try detection/fallbacks (same logic as changeRole)
  const ep = await detectEndpoints();
  ...
  for (const a of attempts) {
    const baseUrl = interpolateId(a.url, userId);
    let body = null;
    if (a.type === "patch_flags") body = { is_employee: true }; // mark approved flag
...
async function loadChat(teamId) {
  ...
  try {
    ...
    if (!Array.isArray(msgs) || !msgs.length) {
      ...
      if (!localMsgs.length) box.innerHTML = "<em>Nu există mesaje.</em>";
      else localMsgs.forEach(renderChatMessage);
      return;
    }
    box.innerHTML = "";
    msgs.forEach((m) => {
    ...
    const localMsgs = readLocalChat(teamId);
    if (!localMsgs.length) box.innerHTML = "<em>Eroare la preluarea mesajelor și nu există mesaje locale.</em>";
    else localMsgs.forEach(renderChatMessage);
  }
}
...
async function sendChat(teamId, text) {
  if (!teamId) { alert("Nu este selectată nicio echipă."); return; }
  if (!text) return;
...
/* Employee page helpers (task progress) */
...
async function makeUserAdmin(userId) {
  try {
    const res = await fetch(`${API_BASE}/users/${userId}/make_admin`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    });
    if (!res.ok) throw new Error('Eroare la promovarea userului');
    alert('Userul a devenit admin!');
    loadUsers(); // reîncarcă lista
  } catch (err) {
    console.error(err);
    alert('Nu s-a putut face admin.');
  }
}

const actionsTd = document.createElement('td');
actionsTd.classList.add('ta-right');

// Buton Aproba (doar daca nu e aprobat)
if (!user.approved) {
  const approveBtn = document.createElement('button');
  approveBtn.textContent = 'Aprobă';
  approveBtn.onclick = () => approveUser(user.id);
  actionsTd.appendChild(approveBtn);
}

// Buton Fa Admin (doar daca e aprobat si nu e admin)
if (user.approved && !user.is_admin) {
  const makeAdminBtn = document.createElement('button');
  makeAdminBtn.textContent = 'Fă Admin';
  makeAdminBtn.onclick = () => makeUserAdmin(user.id);
